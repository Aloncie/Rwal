...
protected:
    ...
    void SetUp() override {
        ...
        mockCurlWrapper =
            std::make_shared<testing::NiceMock<MockCurlWrapper>>(*mockLogs, *mockFileSystem);
        ...
        mockKeywords = std::make_shared<testing::NiceMock<MockKeywords>>(
            *mockConfigReader, *mockLogs, *mockFileSystem);
        mockNetworkManager = std::make_shared<testing::NiceMock<MockNetworkManager>>(
            *mockCurlWrapper, *mockConfigReader, *mockLogs);
...
TEST_F(WallpaperManagerTest, Refresh_LogsFetchImageFails) {
    EXPECT_CALL(*mockKeywords, SilentGetKeyword()).WillOnce(Return("test_keyword"));
    EXPECT_CALL(*mockNetworkManager, fetchImage("test_keyword")).WillOnce(Return(std::nullopt));
    ON_CALL(*mockLogs, writeLogs(_, _, "Failed to fetch image"))
...
TEST_F(WallpaperManagerTest, SaveCurrent_ReturnsSuccess) {
    ...
    EXPECT_CALL(
        *mockFileSystem,
        listDirectory(downloadsDir, rwal::constants::wallpaper::FILE_PREFIX.data()))
        .WillOnce(Return(std::vector<fs::path>{fakeWallpaper}));
