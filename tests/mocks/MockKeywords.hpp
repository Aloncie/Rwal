...
public:
    ...
    MOCK_METHOD(
        std::string, pickRandomKeyword, (const std::vector<std::string>&), (const, override));
    MOCK_METHOD(
        void, promptForKeywords,
        (std::function<void(std::vector<std::string>)>, IUserInterface&, int), (override));
};
