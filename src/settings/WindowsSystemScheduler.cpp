...
WindowsSystemScheduler::WindowsSystemScheduler(Logs& logs, IFileSystem& fs)
    ...
    hr = CoCreateInstance(
        CLSID_TaskScheduler, NULL, CLSCTX_INPROC_SERVER, IID_ITaskService, (void**)&m_pService);
    if (FAILED(hr)) {
...
bool WindowsSystemScheduler::create() {
    ...
    hr = m_pFolder->RegisterTaskDefinition(
        _bstr_t(rwal::constants::names::WIN_TASK_NAME.c_str()), pTask, TASK_CREATE_OR_UPDATE,
...
std::optional<TaskSchedulerType> WindowsSystemScheduler::get() const {
    ...
    if (count > 0) {
        ...
        // It will has more elegant solution later when this logic will be duplicated in another
        // place or become hard mountain
...
std::string WindowsSystemScheduler::set(TaskSchedulerType type) {
    ...
    if (triggersInput == std::nullopt) {
        ...
        if (triggersInput == std::nullopt) {
            m_logs.writeLogs(
                lvl::Error, mod::Scheduler, "Failed to get task triggers after creation");
            return failedLog;
        }
    }
    ...
    if (type != TaskSchedulerType::None) {
        ...
        if (type == TaskSchedulerType::Daily) {
            ...
            if (FAILED(hr)) {
                m_logs.writeLogs(
                    lvl::Error, mod::Scheduler, "Failed to get daily trigger interface");
                return failedLog;
            }
            hr = pDaily->put_DaysInterval(1);
            if (FAILED(hr)) {
                m_logs.writeLogs(
                    lvl::Error, mod::Scheduler, "Failed to set daily trigger interval");
                return failedLog;
            }
            ...
            if (FAILED(hr)) {
                m_logs.writeLogs(
                    lvl::Error, mod::Scheduler, "Failed to set repetition pattern interval");
                return failedLog;
            }
        }
    }
    ...
    if (definitionInput == std::nullopt)
        return failedLog;

    ITaskDefinitionPtr pDef = definitionInput.value();
    hr = m_pFolder->RegisterTaskDefinition(
        _bstr_t(rwal::constants::names::WIN_TASK_NAME.c_str()), pDef, TASK_UPDATE, _variant_t(),
        _variant_t(), TASK_LOGON_INTERACTIVE_TOKEN, _variant_t(L""), &pUpdatedTask);
    if (FAILED(hr)) {
