...
WindowsSystemSchedule::WindowsSystemSchedule(Logs& logs) : m_logs(logs){
	...
	if (FAILED(hr)) {
		m_logs.writeLogs(lvl::Error, mod::Schedule, "Failed to initialize COM");
		return;
	}

	hr = CoCreateInstance(CLSID_TaskScheduler, NULL, CLSCTX_INPROC_SERVER, IID_ITaskService, (void**)&m_pService);
	if (FAILED(hr)) {
		m_logs.writeLogs(lvl::Error, mod::Schedule, "Failed to create COM instance");
		return;
	}
	// Connect to server
	hr = m_pService->Connect(VARIANT(), VARIANT(), VARIANT(), VARIANT());
	if (FAILED(hr)) {
		m_logs.writeLogs(lvl::Error, mod::Schedule, "Failed to connect to server");
		return;
	}
	// Get root folder
	hr = m_pService->GetFolder(_bstr_t(L"\\"), &m_pFolder);
	if (FAILED(hr)) {
		m_logs.writeLogs(lvl::Error, mod::Schedule, "Failed to get root folder");
		return;
	}
	if (m_pFolder == nullptr || m_pService == nullptr) {
		m_logs.writeLogs(lvl::Error, mod::Schedule, "Failed to get root folder or service");
		return;
	}
	m_logs.writeLogs(lvl::Info, mod::Schedule, "Schedule initialized");
}
...
bool WindowsSystemSchedule::status() const {
	...
	if (FAILED(hr)) {
		m_logs.writeLogs(lvl::Error, mod::Schedule, "Failed to get task status");
		return false;
	}
	VARIANT_BOOL enabled;
	hr = pTask->get_Enabled(&enabled);
	if (FAILED(hr)) {
		m_logs.writeLogs(lvl::Error, mod::Schedule, "Failed to check task enabled status");
		return false;
	}
...
bool WindowsSystemSchedule::create() {
	m_logs.writeLogs(lvl::Info, mod::Schedule, "Try to create task");
	ITaskDefinitionPtr pTask;
	
	HRESULT hr = m_pService->NewTask(0, &pTask);
	if (FAILED(hr)) {
		m_logs.writeLogs(lvl::Error, mod::Schedule, "Failed to create task");
		return false;
	}
	...
	if (FAILED(hr)) {
		m_logs.writeLogs(lvl::Error, mod::Schedule, "Failed to get task actions");
		return false;
	}
	...
	if (FAILED(hr)) {
		m_logs.writeLogs(lvl::Error, mod::Schedule, "Failed to create exec action");
		return false;
	}
	hr = pExecAction->put_Path(_bstr_t(rwal::constants::files::EXEC_FILE.c_str()));
	if (FAILED(hr)) {
		m_logs.writeLogs(lvl::Error, mod::Schedule, "Failed to set exec action path");
		return false;
	}
    hr = pExecAction->put_Arguments(_bstr_t(L"change"));
	if (FAILED(hr)) {
		m_logs.writeLogs(lvl::Error, mod::Schedule, "Failed to set exec action arguments");
		return false;
	}
	...
	if (FAILED(hr)) {
		m_logs.writeLogs(lvl::Error, mod::Schedule, "Failed to register task");
		return false;
	}
	m_logs.writeLogs(lvl::Info, mod::Schedule, "Task created successfully");
	return true;
}

bool WindowsSystemSchedule::start() const {
	m_logs.writeLogs(lvl::Info, mod::Schedule, "Try to start task");
	IRegisteredTaskPtr pTask;
	HRESULT hr = m_pFolder->GetTask(_bstr_t(rwal::constants::names::WIN_TASK_NAME.c_str()), &pTask);
	if (FAILED(hr)) {
		m_logs.writeLogs(lvl::Error, mod::Schedule, "Failed to get task");
		return false;
	}
	VARIANT_BOOL enabled;
	hr = pTask->get_Enabled(&enabled);
	...
	if (FAILED(hr)){
		m_logs.writeLogs(lvl::Error, mod::Schedule, "Failed to check task state");
		return false;
	} 
	if (enabled == VARIANT_TRUE) {
        m_logs.writeLogs(lvl::Info, mod::Schedule, "Task already enabled");
        return true;
    }
	...
	if (FAILED(thr)) {
		m_logs.writeLogs(lvl::Error, mod::Schedule, "Failed to check enabled task status");
		return false;
	}
	if (FAILED(hr) || enabled != VARIANT_TRUE) {
		m_logs.writeLogs(lvl::Error, mod::Schedule, "Failed to enable task");
		return false;
	}

	m_logs.writeLogs(lvl::Info, mod::Schedule, "Task enabled successfully");
	return true;
}
...
bool WindowsSystemSchedule::reload() {
    ...
    if (pTask != nullptr) {
		if (SUCCEEDED(hr)){
			...
			if (FAILED(hr)) {
				m_logs.writeLogs(lvl::Error, mod::Schedule, "Failed to delete task");
				return false;
			}
			if (!create()){
				m_logs.writeLogs(lvl::Error, mod::Schedule, "Failed to create task");
			...
			m_logs.writeLogs(lvl::Info, mod::Schedule, "Task reloaded successfully");
			return true;
		}
		m_logs.writeLogs(lvl::Error, mod::Schedule, "Failed to reload task");
		return false;
    }
	m_logs.writeLogs(lvl::Error, mod::Schedule, "Task not found");
	return false;
}
...
bool WindowsSystemSchedule::disable() const {
	...
	if (FAILED(hr)) {
		m_logs.writeLogs(lvl::Error, mod::Schedule, "Failed to disable task");
		return false;
	}
...
std::string WindowsSystemSchedule::get() const {
	...
	m_logs.writeLogs(lvl::Debug, mod::Schedule, "Try to get task schedule");
	auto TaskType = TaskScheduleType::None;

	// Check for the disabled task before any other actions.
	bool enabled = status();
	if (!enabled){
		m_logs.writeLogs(lvl::Info, mod::Schedule, "Task isn't active");
		return "None";
	}
	...
	if (FAILED(hr)){
        m_logs.writeLogs(lvl::Error, mod::Schedule, "Failed to get trigger count");
        return "Error";
    }
    ...
    if (count > 0) {
		...
		if (FAILED(hr)){
			m_logs.writeLogs(lvl::Error, mod::Schedule, "Failed to get trigger");
			return "Error";
		}
		...
		if (FAILED(hr)){
			m_logs.writeLogs(lvl::Error, mod::Schedule, "Failed to get trigger type");
			return "Error";
		}
        ...
        if (type == TASK_TRIGGER_DAILY) {
			m_logs.writeLogs(lvl::Info, mod::Schedule, "Daily trigger found");
			TaskType = TaskScheduleType::Daily;
		}
        else if (type == TASK_TRIGGER_TIME) {
			m_logs.writeLogs(lvl::Info, mod::Schedule, "Hourly trigger found");
			TaskType = TaskScheduleType::Hourly;
		} else {
			m_logs.writeLogs(lvl::Info, mod::Schedule, "Unknown trigger type");
			return "Unknown";
		}
    } else{
		m_logs.writeLogs(lvl::Info, mod::Schedule, "No triggers found");
		return "Not found";
	}
...
std::string WindowsSystemSchedule::set(const std::string& value) {
	...
	m_logs.writeLogs(lvl::Debug, mod::Schedule, "Try to set task");
	...
	if (triggersInput == std::nullopt){
		...
		if (triggersInput == std::nullopt){
			m_logs.writeLogs(lvl::Error, mod::Schedule, "Failed to get task triggers after creation");
			return failedLog;
		}
	}
	...
	if (FAILED(hr)){
		m_logs.writeLogs(lvl::Error, mod::Schedule, "Failed to clear triggers");
		return failedLog;
	}
    ...
    if (type != TaskScheduleType::None) {
        ...
        if (type == TaskScheduleType::Daily) {
			...
			if (FAILED(hr)){
				m_logs.writeLogs(lvl::Error, mod::Schedule, "Failed to create daily trigger");
				return failedLog;
			}
            IDailyTriggerPtr pDaily;
            hr = pTrigger->QueryInterface(IID_IDailyTrigger, (void**)&pDaily);
			if (FAILED(hr)){
				m_logs.writeLogs(lvl::Error, mod::Schedule, "Failed to get daily trigger interface");
				return failedLog;
			}
            hr = pDaily->put_DaysInterval(1);
			if (FAILED(hr)){
				m_logs.writeLogs(lvl::Error, mod::Schedule, "Failed to set daily trigger interval");
				return failedLog;
			}
        } 
			...
			if (FAILED(hr)){
				m_logs.writeLogs(lvl::Error, mod::Schedule, "Failed to create hourly trigger");
				return failedLog;
			}
            IRepetitionPatternPtr pRep;
            hr = pTrigger->get_Repetition(&pRep);
			if (FAILED(hr)){
				m_logs.writeLogs(lvl::Error, mod::Schedule, "Failed to get repetition pattern");
				return failedLog;
			}
            hr = pRep->put_Interval(_bstr_t(L"PT1H"));
			if (FAILED(hr)){
				m_logs.writeLogs(lvl::Error, mod::Schedule, "Failed to set repetition pattern interval");
				return failedLog;
			}
        }
    }
	...
	if (FAILED(hr)){
        m_logs.writeLogs(lvl::Error, mod::Schedule, "Failed to update task");
        return failedLog;
    }
...
std::optional<ITriggerCollectionPtr> WindowsSystemSchedule::getTaskTriggers() const {
	...
	if (FAILED(hr)){
		m_logs.writeLogs(lvl::Error, mod::Schedule, "Failed to get triggers");
		return std::nullopt;
	}
	return pTriggers;
}
...
std::optional<ITaskDefinitionPtr> WindowsSystemSchedule::getTaskDefinition() const {
	...
	if (FAILED(hr)){
		m_logs.writeLogs(lvl::Error, mod::Schedule, "Failed to get task definition");
		return std::nullopt;
	}
	return pDef;
}
