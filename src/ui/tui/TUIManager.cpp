...
bool TUIManager::isInputActive() const {
    return inputActive;
}
...
void TUIManager::showMessage(std::string_view message) {
    ...
    // Message color
    int colorPair = 0;
    if (lower.find("success") != std::string::npos)
        colorPair = 1;
    else if (lower.find("failed") != std::string::npos || lower.find("error") != std::string::npos)
        colorPair = 2;
...
void TUIManager::requestInput(
    std::function<void(std::string)> callback, std::optional<std::string> message) {
    if (message) {
        showMessage(*message);
    }
    if (inputActive)
