...
CurlWrapper::CurlWrapper(Logs& logs, IFileSystem& fs)
    : m_logs(logs), m_fs(fs), curl(curl_easy_init(), CurlDeleter{}) {
...
void CurlWrapper::getRequest(const std::string& url) {
    ...
    if (curl) {
        ...
        if (http_code != 200) {
            m_logs.writeLogs(
                lvl::Error, mod::Network, "Error HTTP code: " + std::to_string(http_code));
            return;
        }
        ...
        try {
            ...
            m_logs.writeLogs(
                lvl::Error, mod::Network, "JSON parse error: " + std::string(e.what()));
            return;
        }
    } else {
        m_logs.writeLogs(lvl::Error, mod::Network, "Failed to init CURL");
        ...
        if (j[paragraph].is_array()) {
            for (auto& item : j[paragraph]) {
                if (item.contains(str)) {
                    if (item[str].is_string()) {
                        m_logs.writeLogs(
                            lvl::Debug, mod::Network,
                            "Data of JSON: " + item[str].get<std::string>());
                        return item[str].get<std::string>();
                    }
                    m_logs.writeLogs(
                        lvl::Debug, mod::Network,
                        "Data of JSON: " + std::to_string(item[str].get<int>()));
            ...
            if (j[paragraph].contains(str)) {
                if (j[paragraph][str].is_string()) {
                    m_logs.writeLogs(
                        lvl::Debug, mod::Network,
                        "Data of JSON: " + j[paragraph][str].get<std::string>());
                    return j[paragraph][str].get<std::string>();
                }

                m_logs.writeLogs(
                    lvl::Debug, mod::Network,
                    "Data of JSON: " + std::to_string(j[paragraph][str].get<int>()));
...
std::optional<fs::path> CurlWrapper::downloadImage(const std::string& image_url) {
    ...
    if (res != CURLE_OK) {
        m_logs.writeLogs(
            lvl::Error, mod::Network,
            "Failed to download image: " + std::string(curl_easy_strerror(res)));
        return std::nullopt;
    }

    m_logs.writeLogs(
        lvl::Info, mod::Network, "Successful download image: " + wallpaper_path.string());
