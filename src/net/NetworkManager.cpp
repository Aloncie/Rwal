...
bool NetworkManager::isAvailable() {
    ...
    m_logs.writeLogs(
        lvl::Warning, mod::Network, "Internet check: FAILED (No connection to 8.8.8.8:53)");
    return false;
#endif
}
...
std::optional<fs::path> NetworkManager::fetchImage(std::string_view keyword) {
    ...
    if (!isAvailable()) {
        m_logs.writeLogs(
            lvl::Info, mod::Network, "Network isn't available, using local wallpapers.");
        fs::path pictures_path = m_fs.getPicturesLocation() / APP_NAME;
        std::string error = m_fs.getLastError();
        if (error != "") {
            m_logs.writeLogs(
                lvl::Error, mod::Network, "Failed to get pictures directory: " + error);
            return std::nullopt;
        }
    ...
    if (search.is_null()) {
        ...
        try {
            ...
            m_logs.writeLogs(
                lvl::Error, mod::Network, "Failed to stoi pageCount: " + std::string(e.what()));
            last_page = 1;
        }
