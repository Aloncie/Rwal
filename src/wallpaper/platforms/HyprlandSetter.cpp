...
bool HyprlandSetter::setWallpaper(const fs::path& path) {
    ...
    if (std::getenv("HYPRLAND_INSTANCE_SIGNATURE") == nullptr) {
        m_logs.writeLogs("Skipping Hyprland: environment variable HYPRLAND_INSTANCE_SIGNATURE not found.");
        return false;
    }
    ...
    if (hasTool("hyprpaper")) {
        ...
        if (std::system(("hyprctl hyprpaper preload " + pathStr).c_str()) != 0) {
            m_logs.writeLogs("Failed to preload wallpaper: " + pathStr);
            return false;
        }
        ...
        if (std::system(("hyprctl hyprpaper wallpaper \" , " + pathStr + "\"").c_str()) != 0) {
            m_logs.writeLogs("Failed to set wallpaper: " + pathStr);
            return false;
        }
		...
		if (std::system("hyprctl reload") != 0) {
			m_logs.writeLogs("Failed to reload Hyprland after setting wallpaper: " + pathStr);
			return false;
		}

        m_logs.writeLogs("Wallpaper set via hyprpaper: " + pathStr);
        return true;
        ...
        if (std::system(("swww img " + pathStr).c_str()) == 0) {
            m_logs.writeLogs("Wallpaper set via swww: " + pathStr);
            return true;
        }
        m_logs.writeLogs("Failed to set wallpaper via swww: " + pathStr);
        return false;
    }

    m_logs.writeLogs("No Hyprland tools (hyprpaper/swww) found.");
    return false;
}
