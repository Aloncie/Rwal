...
bool HyprlandSetter::setWallpaper(const fs::path& path) {
    ...
    if (std::getenv("HYPRLAND_INSTANCE_SIGNATURE") == nullptr) {
        m_logs.writeLogs(rwal::logs::types::Debug, rwal::logs::modules::Wallpaper, "Skipping Hyprland: environment variable HYPRLAND_INSTANCE_SIGNATURE not found.");
        return false;
    }
    ...
    if (hasTool("hyprpaper")) {
        ...
        if (std::system(("hyprctl hyprpaper preload " + pathStr).c_str()) != 0) {
            m_logs.writeLogs(rwal::logs::types::Error, rwal::logs::modules::Wallpaper, "Failed to preload wallpaper: " + pathStr);
            return false;
        }
        ...
        if (std::system(("hyprctl hyprpaper wallpaper \" , " + pathStr + "\"").c_str()) != 0) {
            m_logs.writeLogs(rwal::logs::types::Error, rwal::logs::modules::Wallpaper, "Failed to set wallpaper: " + pathStr);
            return false;
        }
		...
		if (std::system("hyprctl reload") != 0) {
			m_logs.writeLogs(rwal::logs::types::Error, rwal::logs::modules::Wallpaper, "Failed to reload Hyprland after setting wallpaper: " + pathStr);
			return false;
		}

        m_logs.writeLogs(rwal::logs::types::Info, rwal::logs::modules::Wallpaper, "Wallpaper set via hyprpaper: " + pathStr);
        return true;
        ...
        if (std::system(("swww img " + pathStr).c_str()) == 0) {
            m_logs.writeLogs(rwal::logs::types::Info, rwal::logs::modules::Wallpaper, "Wallpaper set via swww: " + pathStr);
            return true;
        }
        m_logs.writeLogs(rwal::logs::types::Error, rwal::logs::modules::Wallpaper, "Failed to set wallpaper via swww: " + pathStr);
        return false;
    }

    m_logs.writeLogs(rwal::logs::types::Warning, rwal::logs::modules::Wallpaper, "No Hyprland tools (hyprpaper/swww) found.");
    return false;
}
