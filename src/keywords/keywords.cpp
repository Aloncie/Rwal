...
std::string Keywords::SilentGetKeyword() {
	...
	if (keywords.empty()) {
		Default(keywords);
		m_logs.writeLogs(lvl::Warning, mod::Keywords, "No keywords found in config. Using default keywords.");
	}
...
void Keywords::editKeywords(IUserInterface& ui) {
	...
	if (ifile.is_open()) {
		...
		m_logs.writeLogs(lvl::Error, mod::Keywords, "Failed opening keywords.txt in " + temp_path.string());
