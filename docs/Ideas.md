...
- [ ] **Value Objects over Strings:** Introduce `Url` and `Path` classes to avoid passing raw strings between `MyCurl` and `WallpaperManager`.
...
- [ ] **Refactor MyCurl:** Remove file-naming logic (suffixes, prefixes) from `MyCurl`. 
